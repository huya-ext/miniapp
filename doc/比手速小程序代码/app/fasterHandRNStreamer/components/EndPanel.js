...
export default class EndPanel extends Component {
  ...
  render() {
    ...
    console.log('endpanel', options, newOptions);
      ...
      <View style={styles.endPanel}>
        <View style={styles.listUl}>
          {
            newOptions.map((item,index, arr) => {
              const oname = (item.optionName && item.optionName.length > 4) ? (item.optionName.slice(0,4) + '...') : item.optionName
        ...
        <View style={styles.navWrap}>
          {
             options && options.map((item,index) => {
              const name = (item.optionName && item.optionName.length > 4) ? item.optionName.slice(0,4) : item.optionName
              return (
        ...
        <TouchableWithoutFeedback onPress={this.reStart}>
          <View style={styles.reStartBtnWrapper}>
            <Text style={styles.reStartBtn}>重新开始</Text>
          </View>
        </TouchableWithoutFeedback>
      </View>
    )
