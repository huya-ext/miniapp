...
class HyExtDev extends React.Component {
      ...
      <div>
        ...
        <Collapse defaultActiveKey={['0']}>
          ...
          <Panel header='调用EBS接口'>
            <RequestForm request={this.request.bind(this)} />
          ...
          <Panel header='礼物相关'>
            <GiftForm
              ...
              getGiftConf={this.getGiftConf.bind(this)}
              sendGift={this.sendGift.bind(this)}
            />
          ...
          <Panel header='直播间相关'>
            <div style={{ marginBottom: '8px' }}>
              ...
              <Button style={{ marginBottom: '8px' }} block onClick={this.getStreamerInfo.bind(this)}>获取主播信息</Button>
            </div>
          ...
          <Panel header='文件相关'>
            <Button style={{ marginBottom: '8px' }} block onClick={this.uploadImg.bind(this)}>上传图片</Button>
            <List bordered dataSource={this.state.log12} renderItem={renderItem} />
          </Panel>
        </Collapse>
      </div>
    )
  ...
  request (options) {
    this.log2('发送请求：' + JSON.stringify(options))
    window.hyExt.request(options).then(rsp => {
  ...
  getGiftConf () {
    this.log5('获取礼物配置')
    window.hyExt.context.getGiftConf().then(giftConf => {
      this.log5('获取成功：' + JSON.stringify(giftConf))
    }).catch(err => {
      this.log5('获取失败：' + err.message)
    })
  }
  sendGift (options) {
    this.log5('指定礼物送礼：' + JSON.stringify(options))
    window.hyExt.context.sendGift(options).then(() => {
      this.log5('送礼成功')
    }).catch(err => {
      this.log5('送礼失败：' + err.message)
    })
  }
  leadBarrage () {
    this.log6('调用引导发言接口')
  ...
  getStreamerInfo () {
    this.log10('获取主播信息')
    window.hyExt.context.getStreamerInfo().then(streamerInfo => {
      this.log10('获取成功：' + JSON.stringify(streamerInfo))
    }).catch(err => {
      this.log10('获取失败：' + err.message)
    })
  }
  ...
  uploadImg () {
    this.log12('上传图片')
    window.hyExt.fs.uploadImg().then(({ url }) => {
      this.log12('上传成功，图片地址为：' + url)
    }).catch(err => {
      this.log12('上传失败：' + err.message)
    })
  }
}
